...
Mat hog(const Mat &Img)
{
    ...
    for(int m=0; m < N_DIVS; m++)
    {
        ...
        {
             for(int i=0; i<cellx; i+=cellx)
             {
                for(int j=0; j<celly; j+=celly)
                {
                    ...
                    px = static_cast<float>(Ix.at<short>((m*cellx)+i, (n*celly)+j ));
                    py = static_cast<float>(Iy.at<short>((m*cellx)+i, (n*celly)+j ));
